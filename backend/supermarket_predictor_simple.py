...
class SimpleSupermarketPredictor:
    def __init__(self):
        self.pipeline = None
    ...
    def load_models(self):
        ...
        try:
            # Get the current directory and navigate to model files
            current_dir = Path(__file__).parent
            model_dir = current_dir.parent.parent / "model" / "super_market" / "saved_models"
            ...
            leakage_path = model_dir / "leakage_encoder.pkl"
            anomaly_path = model_dir / "anomaly_encoder.pkl"
            
            print(f"Pipeline exists: {pipeline_path.exists()}")
            print(f"Leakage encoder exists: {leakage_path.exists()}")
            print(f"Anomaly encoder exists: {anomaly_path.exists()}")
            
            self.pipeline = joblib.load(pipeline_path)
            self.leakage_encoder = joblib.load(leakage_path)
            self.anomaly_encoder = joblib.load(anomaly_path)
    ...
    def preprocess_data(self, df):
        """Preprocess input data for prediction - extracted from notebook"""
        try:
