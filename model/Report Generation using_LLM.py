...
def create_revenue_leakage_report(api_key, file_path):
    try:
        ...
        # --- NEW: Set the conversion rate ---
        # Note: This is a static value. For a dynamic rate, you would need to use a financial API.
        USD_TO_INR_RATE = 87.79
        
        # Load the dataset
        df = pd.read_csv(file_path)

        # Filter for data with 'Anomaly' leakage
        leakage_data = df[df['Leakage_Flag'] == 'Anomaly']
        ...
        leakage_sample = leakage_data[[
            ...
            'Transaction_Type',
            'Mode_of_Payment'
        ]].head(20)  # Changed from 10 to 20

        leakage_sample_str = leakage_sample.to_string(index=False)
        
        # --- ROOT CAUSE ANALYSIS DATA ---
        # Group by Anomaly Type and Store Branch to find patterns
        grouped_by_anomaly_branch = leakage_data.groupby(['Anomaly_Type', 'Store_Branch'])['Balance_Amount'].sum().reset_index()
        
        # Convert grouped balance amounts to INR
        grouped_by_anomaly_branch['Balance_Amount_INR'] = grouped_by_anomaly_branch['Balance_Amount'] * USD_TO_INR_RATE
        grouped_by_anomaly_branch_str = grouped_by_anomaly_branch[['Anomaly_Type', 'Store_Branch', 'Balance_Amount_INR']].to_string(index=False)

        # Find the top 5 products and cashiers with the most leakage incidents
        ...
        leakage_percentage = (leakage_data['Balance_Amount'].sum() / total_billed_amount) * 100

        # Calculate total amounts for key fields in the leakage data in INR
...
*Report Sections:*
- *Leakage Summary*: Identify the main reasons ('Anomaly_Type'), locations ('Store_Branch'), and dates of leakage.
...
"""
        # Create a Gemini model instance and generate the report content
...
# This is where you need to provide your actual Google Gemini API key and file path.
# Replace "YOUR_API_KEY" with your API key.
create_revenue_leakage_report(api_key="YOUR_API_KEY", file_path="supermarket_dataset.csv")
